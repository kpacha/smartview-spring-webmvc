...
/**
 * A file view supporting resume of downloads (required for audio and video
 * streaming) and client-side caching and GZIP of text content. This view can
 * also be used for images, client-side caching would become more efficient.
 * This view can also be used for text files, GZIP would decrease network
 * bandwidth.
 * 
 * Based on the resumible download serlvet posted by BalusC (see the link)
 * 
 * @author kpacha
 * @author BalusC
 * @link 
...
public class DownloadView extends AbstractUrlBasedView {
    ...
    protected void renderMergedOutputModel(Map<String, Object> model,
	    ...
	    throws Exception {
	if (!model.containsKey(FILE_PATH_KEY)) {
	    throw new Exception("File not found");
	}
	// TODO set this as a property or param
	setBasePath("/tmp/");
	processRequest((String) model.get(FILE_PATH_KEY), request, response,
     ...
     * @throws Exception
     */
    private void setBasePath(final String basePath) throws Exception {
	// Validate base path.
	if (basePath == null) {
	    throw new Exception(
		...
		throw new Exception(this.getClass().getSimpleName()
			+ " init param 'basePath' value '" + basePath
     ...
     */
    private void processRequest(String requestedFile,
	    HttpServletRequest request, HttpServletResponse response,
	...
	File file = validateRequestFile(requestedFile, response);
	if (file == null) {
	    return;
	}
	...
	if (!getContentType().equals(contentType)) {
		...
		logger.debug("Isn't [" + fileName + " (" + contentType
			+ ")] a valid file type? Expected: " + getContentType());
	    }
	...
	// set default GZIP support and content disposition.
    ...
    private void send304(HttpServletResponse response, String eTag)
	    throws IOException {
	response.setHeader("ETag", eTag); // Required in 304.
	response.sendError(HttpServletResponse.SC_NOT_MODIFIED);
    }

    private File validateRequestFile(String requestedFile,
